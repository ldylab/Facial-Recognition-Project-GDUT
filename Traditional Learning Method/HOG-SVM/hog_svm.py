...
# 训练集图片的位置
train_image_path = 'train_female'
# 测试集图片的位置
test_image_path = 'val_female'

# 训练集标签的位置
train_label_path = os.path.join('train_female','train_female.txt')
# 测试集标签的位置
test_label_path = os.path.join('val_female','val_female.txt')
...
def train_and_test():
    ...
    for feat_path in glob.glob(os.path.join(test_feat_path, '*.feat')):
        ...
        for feat_path in glob.glob(os.path.join(test_feat_path, '*.feat')):
            data = joblib.load(feat_path)
            test_features.append(data[:-1].astype(np.float64))
            test_labels.append(data[-1].astype())

        test_label_array = np.array(test_labels)
        test_features_array = np.array(test_features)
        print(test_label_array, test_features_array)
        result = clf.predict(data_test_feat)

        disp = plot_confusion_matrix(clf, test_features_array, test_label_array,
                                     display_labels=2,
                                     cmap=plt.cm.Greens)
